...
class PerforceRequests():
    ...
    def get_fol_fi_on_folder( self, type ,folder, pyStAl, server, user, workspace ):
        ...
        if pyStAl == False:
            ...
            file_content = hlp.write_perforce_command_file ( line , True, 'get_perf_fi_dirs.json', server, user, workspace)
    ...
    def workspaces_ls(self, pyStAl, server, user, workspace):
        ...
        if pyStAl == False:
            ...
            file_content = hlp.write_perforce_command_file ( line , True, 'workspace_request.json', server, user, workspace)
    ...
    def pull_file_2_local(self, file, pyStAl, server, user, workspace):
        ...
        if pyStAl == False:
            ...
            file_content = hlp.write_perforce_command_file ( line , True, 'pull_perf_query.json', server, user, workspace)
    ...
    def checkout_file( self, local_path_file ,server, user, workspace, pyStAl=True):
        ...
        if pyStAl == False:
            ...
            file_content = hlp.write_perforce_command_file ( line , True, 'result_perf_query.json', server, user, workspace)
    ...
    def get_all_dir_fol( self, path_root, server, user, workspace, return_ls = [] , pyStAl = False ):
        ...
        if pyStAl == False:
            ...
            for directory in folder_ls:
                ...
                line = '    %s = perf.get_all_dir_fol( "%s", "%s", "%s", "%s", return_ls = [] , pyStAl = False )' %( de.ls_result, 
                                                                                                                ...
                                                                                                                server, user, 
                                                                                                                workspace )
                file_content = hlp.write_perforce_command_file ( line , True, 'get_perf_sub_fold.json', server, user, workspace)
            ...
            line = '    %s = perf.get_all_dir_fol( "%s", "%s", "%s", "%s", return_ls = [] , pyStAl = False )' %( de.ls_result, 
                                                                                                                ...
                                                                                                                server, user, 
                                                                                                                workspace )
            file_content = hlp.write_perforce_command_file ( line , True, 'get_perf_fold.json', server, user, workspace)

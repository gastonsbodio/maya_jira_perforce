...
class MyMainWindow(QMainWindow):
    ...
    def eventFilter(self,obj, event):
        """Not using proccesing information all time user moving on table (heler to create articial hover)
        Args:
            obj ([recursive internal var]): [description]
        ...
        try:
            ...
            modelIndex = self.ui.table_assetsTasks.indexAt(rw) 
            if modelIndex.row() != -1:
                self.ui.table_assetsTasks.setStyleSheet("background-color: rgb(30, 30, 30);"  )
                self.default_color_row( self.ui.table_assetsTasks, modelIndex.row())
            return super(MyMainWindow, self).eventFilter(obj, event)
        except Exception as e:
            pass
            
    def colorize_entire_row(self, table, idx_row, qcolor):
        """Hover artificial effect on the hole row items
        Args:
            table ([qtablewid]): [description]
            idx_row ([int]): [description]
            qcolor ([qcolor]): [description]
        """
        for idx_col in range(table.columnCount()):
            item = QTableWidgetItem('')
            table.setItem( idx_row, idx_col, item )
            item.setBackground(qcolor)

    def default_color_row(self, table, idx_row):
        """Not using, artificially assigning colors to rows
        Args:
        ...
        for row in range(table.rowCount()):
            if row != idx_row:
                if (row % 2) == 0: 
                    self.colorize_entire_row(table, row, QBrush(QColor(54, 54, 54 )))
                else: 
                    self.colorize_entire_row(table, row, QBrush(QColor(45, 45, 45 )))
            else:
                self.colorize_entire_row( table, row, QBrush(QColor(255, 87, 51 )))
    ...
    def load_project_combo(self):
        ...
        proj_ls_diccs = self.jira_m.get_projects( de.SERVER )
    ...
    def load_workspace_combo(self):
        ...
        self.worksp_ls = perf.workspaces_ls( True , self.PERF_SERVER, self.PERF_USER, self.PERF_WORKSPACE)
...
class table_features( ):#QWidget ):
    ...
    def get_task_file(self, asset_na, area):
        ...
        perf = pr.PerforceRequests()
        perf.pull_file_2_local( depot_full_path, True , self.PERF_SERVER, self.PERF_USER, self.PERF_WORKSPACE )
    ...
    def get_thumb_from_depot( self, thumbDepotPath, thumb_fi_na , table , colum_idx ):
        ...
        try:
            ...
            pass
        perf = pr.PerforceRequests()
        perf.pull_file_2_local( thumbDepotPath+thumb_fi_na, True , self.PERF_SERVER, self.PERF_USER, self.PERF_WORKSPACE )
        thumb_fi = os.path.join(  local_path, thumb_fi_na)
        while not os.path.isfile( thumb_fi ):
            time.sleep(0.05)
        ...
        try:
            if ev.current_scene().startswith( thumbLocalPath ):
                if os.path.exists(thumbLocalPath):
                    ...
                    perf.add_and_submit( thumbLocalPath+thumb_fi_na, comment , self.PERF_SERVER, self.PERF_USER, self.PERF_WORKSPACE ,True )
                else:
